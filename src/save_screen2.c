...
screen->width * screen->height * 3))))
	...
	while (++i < screen->height)
	{
		...
		{
			img[i * screen->width * 3 + j * 3] = screen->img_data
[(screen->height - i - 1) * screen->width + j];
			img[i * screen->width * 3 + j * 3 + 1] = (screen->img_data
[(screen->height - i - 1) * screen->width + j] >> 8);
			img[i * screen->width * 3 + j * 3 + 2] = (screen->img_data
[(screen->height - i - 1) * screen->width + j] >> 16);
		}
	}
	return (img);
}
