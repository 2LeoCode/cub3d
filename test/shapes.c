...
{
	return (lst ? (1 + lst_size(lst->next)) : 0);
}
...
int				process_key(int key, t_mlxvar *mlx_var)
{
	...
	if (key == 65362
	&& mlx_var->map[(int)(mlx_var->py + (sin(mlx_var->rot) / mlx_var->box_size_y) * 3)]
	[(int)(mlx_var->px + (cos(mlx_var->rot) / mlx_var->box_size_x) * 3)] == '0') //top
	{
		mlx_var->px += (cos(mlx_var->rot) / mlx_var->box_size_x) * 3;
		mlx_var->py += (sin(mlx_var->rot) / mlx_var->box_size_y) * 3;
	}
	if (key == 65364
	&& mlx_var->map[(int)(mlx_var->py - (sin(mlx_var->rot) / mlx_var->box_size_x) * 3)]
	[(int)(mlx_var->px - (cos(mlx_var->rot) / mlx_var->box_size_x) * 3)] == '0') //bot
	{
		mlx_var->px -= (cos(mlx_var->rot) / mlx_var->box_size_x) * 3;
